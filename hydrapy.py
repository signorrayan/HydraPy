...
class HydraConfig:
    ...
    DEFAULT_WORDLISTS_DIR = Path("wordlists")
...
class HydraAttack:
    ...
    def validate_target(self, ip: Optional[str], hostname: Optional[str]) -> Optional[str]:
        if ip:
    ...
    async def process_hydra_output(self,
    ...
    def save_results(self, result: AttackResult):
        output_file = self.results_dir / f"attack_{result.protocol}_{result.target}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def run_attack(self,
        ...
        try:
            command = self._build_command(
                validated_target, protocol, login_file, password_file,
                tasks, port, stop_on_success
            )

            logger.info(f"Starting Hydra attack against {validated_target} using {protocol}")
            ...
            try:
                stdout_result, stderr_result = await asyncio.wait_for(
                    asyncio.gather(
                        self.process_hydra_output(process.stdout, False, process, found_credentials),
                        self.process_hydra_output(process.stderr, True, process, found_credentials)
                    ),
                    timeout=timeout
                )
            ...
            extracted_port = found_credentials[0].pop("port", None) if found_credentials else None
    ...
    def _build_command(self, target, protocol, login_file, password_file, tasks, port, stop_on_success):
        login_file = login_file or self.config.get_default_wordlist_path(protocol, "logins")
        password_file = password_file or self.config.get_default_wordlist_path(protocol, "passwords")
...
def setup_argparse():
    ...
    parser.add_argument("--protocol", required=True, help="Protocol to attack (e.g., ssh, ftp, postgres)")
    ...
    parser.add_argument("-t", "--tasks", type=int, default=16, help="Number of parallel tasks")
    parser.add_argument("--timeout", type=int, default=30, help="Attack timeout in seconds")
    parser.add_argument("--continue-on-success", action="store_true", help="Continue after finding credentials")
    return parser
